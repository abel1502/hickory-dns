...
/// Utility to encode no queries for use in responses that have no queries.
pub struct NoQueries;

impl NoQueries {
    /// Helper for encoding no queries in a MessageResponse.
    pub fn as_emit_and_count(&self) -> QueriesEmitAndCount<'_> {
        QueriesEmitAndCount {
            queries: false,
            cached_serialized: &[],
...
pub struct QueriesEmitAndCount<'q> {
    queries: bool,
